...
from model.model import WiperTripPredictor
import app.templates as T
from agent.advisor_chat import render_advisor_chat
...
if st.session_state.mode == "Manual" and not st.session_state.running:
    ...
    with s3:
        if st.button("STEP x10 ▶▶", width="stretch"):
...
main_left, main_right = st.columns([5.5, 2])

with main_left:
    left_main_placeholder = st.empty()
    chat_container = st.container()

with main_right:
    right_main_placeholder = st.empty()
...
def render_dashboard(idx: int):
    ...
    # ================================================================
    with top_bar_placeholder.container():
        # ---- Top Bar ----
        st.markdown(
    ...
    with left_main_placeholder.container():
        # ---- Left & Center Columns ----
        ...
        st.markdown(T.section_title("Event Log", margin_top=16), unsafe_allow_html=True)
        st.markdown(
            T.event_log(st.session_state.event_log),
            unsafe_allow_html=True,
        )

    with right_main_placeholder.container():
        ...
        st.markdown(T.section_title("Model Information", margin_top=16), unsafe_allow_html=True)
        st.markdown(
