...
def load_data(path: str, subsample: int = 6) -> pd.DataFrame:
    ...
    df = df.iloc[:len(df)//subsample].reset_index(drop=True)

    # Parse time (CSV truncates seconds, so we reconstruct the 10s intervals)
    df["Time"] = pd.to_datetime(df["Time"], format="mixed", dayfirst=False)
    df["Time"] = df["Time"] + pd.to_timedelta((df.groupby("Time").cumcount() * 10), unit="s")
